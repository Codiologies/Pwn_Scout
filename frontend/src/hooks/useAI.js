...
export function useAI() {
  ...
  const downloadModel = useCallback(async (modelKey = 'smart', silent = false) => {
    ...
    try {
      ...
      localStorage.setItem('scout_model_accepted', '1');
      localStorage.setItem('scout_model_key', modelKey);
      setAIState(s => ({
        ...s,
        status: 'ready',
        source: 'webllm',
  ...
  useEffect(() => {
    if (localStorage.getItem('scout_model_accepted') === '1' && !isModelLoaded()) {
      const savedKey = localStorage.getItem('scout_model_key') || 'smart';
